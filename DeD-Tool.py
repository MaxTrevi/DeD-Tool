...
class DeDToolGUI:
    ...
    def show_chat(self):
        ...
        # Tab Chat Segreta (rimane, ma modificata per funzionare come lista conversazioni)
        ...
        # Quando cambio tab rimuovo l'indicazione numerica e aggiorno badge
        def on_tab_changed(event):
            ...
            # Se è il TAB segreto, ricarico la lista contatti
    ...
    def _count_unread_by_category_fast(self):
        """Conta messaggi non letti secondo le NUOVE regole della chat segreta unificata."""
        ...
        try:
            # CHAT COMUNE = is_secret = 0 e receiver NULL
            cursor.execute("""
                SELECT COUNT(*) AS comune
                FROM chat_messages m
                LEFT JOIN chat_reads r 
                       ON m.id = r.message_id AND r.user_id = %s
                WHERE r.id IS NULL
                  AND m.receiver_id IS NULL
                  AND m.is_secret = 0
            """, (user_id,))
            comune = cursor.fetchone()['comune'] or 0

            # CORREZIONE: CHAT SEGRETE = solo messaggi RICEVUTI non letti
            cursor.execute("""
                SELECT COUNT(*) AS segreti
                FROM chat_messages m
                LEFT JOIN chat_reads r 
                       ON m.id = r.message_id AND r.user_id = %s
                WHERE r.id IS NULL
                  AND m.is_secret = 1
                  AND m.receiver_id = %s  -- Solo messaggi dove IO sono il destinatario
            """, (user_id, user_id))
            segreti = cursor.fetchone()['segreti'] or 0

            # Chat privata eliminata → sempre 0
            privati = 0
    ...
    def create_secret_chat_interface_fast(self, parent):
        ...
        # --- DOPPIO CLICK CORRETTO ---
        def on_tree_double_click(event):
            item = tree.identify_row(event.y)
            if item:
                tree.selection_set(item)
                self._open_selected_secret_conversation(tree)
    ...
    def _load_secret_conversations_list(self, tree):
        """Popola la treeview con lista contatti + ultimo messaggio + conteggio non letti."""
        try:
            # svuota
            ...
            # Lista contatti: per DM => tutti i giocatori; per giocatore => altri giocatori + DM
            if self.current_user['role'] == 'DM':
                cursor.execute("SELECT id, username FROM users WHERE role = %s ORDER BY username", ('GIOCATORE',))
                contacts = cursor.fetchall()
            else:
                # trova il DM (primo) — se non esiste, solo altri giocatori
                cursor.execute("SELECT id, username FROM users WHERE role = 'DM' ORDER BY id LIMIT 1")
                dm = cursor.fetchone()
                # altri giocatori
                cursor.execute("SELECT id, username FROM users WHERE role = 'GIOCATORE' AND id != %s ORDER BY username", (self.current_user['id'],))
                players = cursor.fetchall()
                contacts = players
                if dm:
                    # fallo comparire in testa
                    contacts = [dm] + contacts

            # Per ogni contatto prendi ultimo messaggio fra current_user e contact (is_secret=1)
            for c in contacts:
                cid = c['id']
                # ultimo messaggio
                cursor.execute("""
                    ...
                    LIMIT 1
                """, (self.current_user['id'], cid, cid, self.current_user['id']))
                last = cursor.fetchone()
                last_text = last['message'][:60] + ("..." if last and len(last.get('message',''))>60 else "") if last else ""

                # CORREZIONE: conta SOLO i messaggi RICEVUTI (inviati dall'altro utente) e non letti
                cursor.execute("""
                    SELECT COUNT(*) as cnt
                    FROM chat_messages m
                    LEFT JOIN chat_reads r ON m.id = r.message_id AND r.user_id = %s
                    WHERE r.id IS NULL
                      AND m.is_secret = 1
                      AND m.sender_id = %s  -- Solo messaggi INVIATI dal contatto
                      AND m.receiver_id = %s  -- Solo messaggi RICEVUTI da me
                """, (self.current_user['id'], cid, self.current_user['id']))
                cnt_row = cursor.fetchone()
                unread = cnt_row['cnt'] if cnt_row else 0

                tree.insert('', 'end', iid=str(cid), values=(c['username'], last_text, unread))
            cursor.close()
        except Exception as e:
            messagebox.showerror("Errore", f"Errore caricamento conversazioni segrete: {e}")

    def _open_secret_chat_window(self, contact_id):
        """
        Apre popup conversazione segreta con contact_id.
        Questo popup ha polling (after) per aggiornare i messaggi ogni 1s.
        """
        try:
            ...
            send_btn = ttk.Button(input_frame, text="📤 Invia", 
            ...
            # funzione per caricare messaggi
            def load_messages_periodic():
                try:
                    ...
                    # individua quali sono non letti per l'utente corrente
                    new_ids = []
                    for m in msgs:
                        cursor.execute("SELECT id FROM chat_reads WHERE user_id = %s AND message_id = %s", (self.current_user['id'], m['id']))
                        if not cursor.fetchone():
                            new_ids.append(m['id'])
                    ...
                    if new_ids:
                        ...
                        """, [self.current_user['id']] + new_ids)
                        self.db.commit()

                    # mostra messaggi (in ordine cronologico)
                    messages_text.config(state='normal')
                    messages_text.delete(1.0, tk.END)
                    for m in reversed(msgs):
                        date_str = m.get('mystara_date', '')
                    ...
                    print(f"Errore caricamento chat segreta popup: {e}")
                # re-schedule (salva l'id in attributo popup)
                try:
                    popup._chat_after_id = popup.after(1000, load_messages_periodic)
                except:
                    pass

            # quando chiudo popup cancella after
            def on_close():
                try:
            ...
            # avvia la prima lettura e poi periodica
            load_messages_periodic()

            # aggiorna la lista conversazioni sul main (rimuove badge)
            try:
                if hasattr(self, 'secret_contacts_tree'):
                    self._load_secret_conversations_list(self.secret_contacts_tree)
            except:
    ...
    def update_chat_button_fast(self):
        """Aggiorna il pulsante chat secondo le NUOVE regole."""
