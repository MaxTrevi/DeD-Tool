...
__VERSION__ = "1.0.2"  # La versione corrente del mio script
...
def show_welcome_message():
    print("=" * 60)
    print("🎲 Benvenuto su D&D Tool V.1.0.2".center(60))
...
class DeDTool:
    ...
    def view_bank_transactions(self):
        ...
        try:
            ...
            # 🔹 Query arricchita con join per utente, PG e banca destinazione
            cursor.execute("""
                SELECT 
            ...
            if not transactions:
                ...
                for tx in transactions:
                    ...
                    operazione = tx['operation_type'].capitalize()
                    importo = f"{tx['amount']:.2f} MO"
                    motivo = tx.get('reason', 'Senza motivo')

                    giocatore = tx['user_name'] if tx['user_name'] else "Sconosciuto"
                    pg = tx['pg_name'] if tx['pg_name'] else "N/A"

                    if tx['operation_type'] == "trasferimento" and tx['target_bank_name']:
                        extra = f" → Destinazione: {tx['target_bank_name']}"
                    else:
                        extra = ""

                    print(f"- [{ded_date}] {operazione} di {importo} | PG: {pg} | Giocatore: {giocatore} | Motivo: {motivo}{extra}")
    ...
    def deposit_funds(self):
        ...
        try:
            ...
            cursor.execute("""
                ...
                (bank_id, pg_id, user_id, target_bank_id, operation_type, amount, reason, timestamp) 
                VALUES (%s, %s, %s, NULL, 'deposito', %s, %s, NOW())
            """, (
                selected_bank['id'],
                selected_bank.get('pg_id'),
                self.current_user['id'],
                amount,
                reason
            ))
    ...
    def withdraw_funds(self):
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO bank_transactions 
                (bank_id, pg_id, user_id, target_bank_id, operation_type, amount, reason, timestamp)
                VALUES (%s, %s, %s, NULL, 'prelievo', %s, %s, NOW())
            """, (
                selected_bank['id'],
                selected_bank.get('pg_id'),
                self.current_user['id'],
    ...
    def transfer_funds(self):
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO bank_transactions 
                (bank_id, pg_id, user_id, target_bank_id, operation_type, amount, reason, timestamp)
                VALUES (%s, %s, %s, %s, 'trasferimento', %s, %s, NOW())
            """, (
                from_bank['id'],
                from_bank.get('pg_id'),
                self.current_user['id'],
    ...
    def scarica_diario_campagna(self):
        self._clear_screen()
        ...
        try:
            ...
            if versione_locale == ultima_versione_online:
                ...
                cursor.execute(sql, (ultima_versione_online, utente['id']))
                self.db.commit()
                cursor.close()
...
"""
    ...
    def send_message(self):
        self._clear_screen()
        print("--- ✉️  Invio Messaggio ---")

        if not self.current_user:
            print("Devi essere loggato per inviare un messaggio.")
            input("\nPremi Invio per continuare...")
            return

        cursor = self.db.cursor(dictionary=True)

        print("1. Chat Comune (visibile a tutti)")
        print("2. Messaggio Privato DM ↔ Giocatore")
        print("3. Messaggio Segreto tra Giocatori")
        choice = input("Seleziona il tipo di messaggio: ").strip()

        receiver_id = None
        is_secret = False

        if choice == '1':
            print("Scrivi un messaggio per la chat comune.")
        elif choice == '2':
            # Solo DM o Giocatore possono scriversi reciprocamente
            if self.current_user['role'] == 'DM':
                cursor.execute("SELECT id, username FROM users WHERE role = 'GIOCATORE'")
            else:
            ...
            users = cursor.fetchall()
            if not users:
                print("Nessun destinatario disponibile.")
                input("\nPremi Invio per continuare...")
                return
            ...
            try:
                sel = int(input("Scegli destinatario: ")) - 1
                receiver_id = users[sel]['id']
            except Exception:
                print("Scelta non valida.")
                input("\nPremi Invio per continuare...")
                return
            ...
            if self.current_user['role'] != 'GIOCATORE':
                print("Solo i giocatori possono inviare messaggi segreti tra loro.")
                input("\nPremi Invio per continuare...")
                return
            cursor.execute("SELECT id, username FROM users WHERE role = 'GIOCATORE' AND id != %s", (self.current_user['id'],))
            players = cursor.fetchall()
            if not players:
                print("Nessun altro giocatore disponibile.")
                input("\nPremi Invio per continuare...")
                return
        ...
        message = input("\nScrivi il messaggio: ").strip()
        if not message:
            print("Messaggio vuoto annullato.")
            input("\nPremi Invio per continuare...")
            return
        ...
        try:
            cursor.execute("""
                ...
                VALUES (%s, %s, %s, %s, %s)
            """, (self.current_user['id'], receiver_id, is_secret, message, ded_date))
            self.db.commit()
    ...
    def view_chat(self):
        while True:
            ...
            counts = self._count_unread_by_category()
            def fmt(label, n):
                return f"{label} ({n} nuovi)" if n > 0 else label

            print("--- 💬 Sistema Chat ---")
            print(f"1. {fmt('Chat Comune', counts['comune'])}")
            print(f"2. {fmt('Messaggi Privati (DM ↔ Giocatore)', counts['privati'])}")
            print(f"3. {fmt('Chat Segrete tra Giocatori', counts['segreti'])}")
            print("0. Torna indietro")
            choice = input("Scegli quale chat visualizzare: ").strip()
            ...
            if choice == '1':
                while True:
                    ...
                    try:
                        ...
                        self._mark_messages_as_read(rows)
                    except Exception as e:
                        print(f"❌ Errore caricando la chat: {e}")
                        input("\nPremi Invio per continuare...")
                        break
                    ...
                    if not rows:
                        ...
                        for msg in rows[-50:]:
                            ...
                            print(f"[{date_str}] {sender}: {msg['message']}")

                    print("\nScrivi un messaggio (o '/exit' per uscire):")
                    new_message = input("✏️ > ").strip()
                    ...
                    if new_message.lower() == '/exit':
                        ...
                        try:
                            cursor.execute("""
                                INSERT INTO chat_messages (sender_id, receiver_id, is_secret, message, mystara_date)
                                VALUES (%s, NULL, 0, %s, %s)
                            """, (self.current_user['id'], new_message, ded_date))
                            self.db.commit()
                        except Exception as e:
                            print(f"❌ Errore durante l'invio: {e}")
                            input("\nPremi Invio per continuare...")

            # ====== CHAT PRIVATA (DM <-> GIOCATORE) ======
            elif choice == '2':
                # Recupera lista utenti possibili
                if self.current_user['role'] == 'DM':
                    cursor.execute("SELECT id, username FROM users WHERE role = 'GIOCATORE'")
                else:
                    cursor.execute("SELECT id, username FROM users WHERE role = 'DM'")
                ...
                # Calcola i non letti per singolo contatto
                unread_per_user = {}
                for u in users:
                    uid = u['id']
                    cursor.execute("""
                    ...
                    """, (self.current_user['id'], uid, self.current_user['id'], self.current_user['id'], uid))
                    row = cursor.fetchone()
                    unread_per_user[uid] = row["unread_count"] if row else 0

                print("\n📜 Destinatari disponibili:")
                for i, u in enumerate(users):
                    unread = unread_per_user.get(u['id'], 0)
                    label = f"{u['username']} ({unread} nuovi)" if unread > 0 else u['username']
                    print(f"{i+1}. {label}")
                print("0. Torna indietro")
                ...
                try:
                    ...
                    if sel == -1:
                        continue
                    receiver_id = users[sel]['id']
                    receiver_name = users[sel]['username']
                except Exception:
                    print("Scelta non valida.")
                    input("\nPremi Invio per continuare...")
                    continue
                ...
                while True:
                    self._clear_screen()
                    print(f"=== 📩 Chat Privata con {receiver_name} ===\n")
                    ...
                    if not rows:
                        ...
                        for msg in rows[-50:]:
                            ...
                            if msg['sender_id'] == self.current_user['id']:
                                print(f"[{date_str}] Tu → {receiver_name}: {msg['message']}")
                            else:
                                print(f"[{date_str}] {sender} → Tu: {msg['message']}")
                    ...
                    if new_message.lower() == '/exit':
                        ...
                        try:
                            cursor.execute("""
                                INSERT INTO chat_messages (sender_id, receiver_id, is_secret, message, mystara_date)
                                VALUES (%s, %s, 0, %s, %s)
                            """, (self.current_user['id'], receiver_id, new_message, ded_date))
                            self.db.commit()
                            # aggiorna il contatore in tempo reale
                            unread_per_user[receiver_id] = 0
                        except Exception as e:
                            print(f"❌ Errore durante l'invio: {e}")
                            input("\nPremi Invio per continuare...")

            # ====== CHAT SEGRETA TRA GIOCATORI (visibile anche al DM) ======
            elif choice == '3':
                if self.current_user['role'] == 'DM':
                    while True:
                        self._clear_screen()
                        print("=== 🤫 Chat Segrete tra Giocatori (solo lettura DM) ===\n")
                        ...
                        try:
                            ...
                            print(f"❌ Errore caricando le chat segrete: {e}")
                            input("\nPremi Invio per continuare...")
                            break

                        if not rows:
                            print("Nessuna chat segreta trovata.\n")
                        else:
                            for msg in rows[-80:]:
                                date_str = msg.get('mystara_date', '')
                                sender = msg.get('sender_name', '?')
                                receiver = msg.get('receiver_name', '?')
                                print(f"[{date_str}] {sender} → {receiver}: {msg['message']}")

                        print("\nPremi Invio per aggiornare o digita '/exit' per uscire.")
                    ...
                    # Aggiungi counter dei non letti per ciascun giocatore
                    unread_per_player = {}
                    for p in players:
                        pid = p['id']
                        cursor.execute("""
                            ...
                            FROM chat_messages m
                            LEFT JOIN chat_reads r ON m.id = r.message_id AND r.user_id = %s
                            WHERE r.id IS NULL AND m.is_secret = 1
                              AND ((m.sender_id = %s AND m.receiver_id = %s)
                                OR (m.sender_id = %s AND m.receiver_id = %s))
                        """, (self.current_user['id'], pid, self.current_user['id'], self.current_user['id'], pid))
                        row = cursor.fetchone()
                        unread_per_player[pid] = row["unread_count"] if row else 0

                    print("\n🎭 Giocatori disponibili:")
                    for i, p in enumerate(players):
                        unread = unread_per_player.get(p['id'], 0)
                        label = f"{p['username']} ({unread} nuovi)" if unread > 0 else p['username']
                        print(f"{i+1}. {label}")
                    print("0. Torna indietro")
                    ...
                    try:
                        ...
                        if sel == -1:
                            continue
                        receiver_id = players[sel]['id']
                        receiver_name = players[sel]['username']
                    except Exception:
                        print("Scelta non valida.")
                        input("\nPremi Invio per continuare...")
                        continue
                    ...
                    while True:
                        self._clear_screen()
                        print(f"=== 🤫 Chat Segreta con {receiver_name} ===\n")
                        ...
                        if new_message.lower() == '/exit':
                            ...
                            try:
                                ...
                                """, (self.current_user['id'], receiver_id, new_message, ded_date))
                                self.db.commit()
                                unread_per_player[receiver_id] = 0
                            except Exception as e:
                                print(f"❌ Errore durante l'invio: {e}")
                                input("\nPremi Invio per continuare...")
    ...
    # --- CHAT: Gestione messaggi letti/non letti ---
    def _mark_messages_as_read(self, chat_rows):
        """Segna i messaggi come letti per l'utente loggato."""
        if not self.current_user or not chat_rows:
            return
        ...
        for msg in chat_rows:
            ...
            """, (self.current_user['id'], msg_id))
            if not cursor.fetchone():
                cursor.execute("""
                    INSERT INTO chat_reads (user_id, message_id) VALUES (%s, %s)
                """, (self.current_user['id'], msg_id))
        self.db.commit()
        cursor.close()
    ...
    def _count_unread_messages(self):
        ...
        try:
            if self.current_user['role'] == 'DM':
                # Il DM vede anche le chat segrete
                cursor.execute("""
                    SELECT COUNT(*)
                    FROM chat_messages m
                    LEFT JOIN chat_reads r ON m.id = r.message_id AND r.user_id = %s
                ...
                # Giocatore: conta solo messaggi pubblici + diretti a lui + segreti che lo coinvolgono
                cursor.execute("""
                    SELECT COUNT(*)
                    FROM chat_messages m
                    LEFT JOIN chat_reads r ON m.id = r.message_id AND r.user_id = %s
                ...
                """, (self.current_user['id'], self.current_user['id'], self.current_user['id'], self.current_user['id'], self.current_user['id']))
            count = cursor.fetchone()[0]
            return count or 0
        finally:
            cursor.close()

    def _count_unread_by_category(self):
        """Restituisce un dizionario con i messaggi non letti divisi per categoria."""
        if not self.current_user:
            return {"comune": 0, "privati": 0, "segreti": 0}

        cursor = self.db.cursor()
        result = {"comune": 0, "privati": 0, "segreti": 0}
        ...
        try:
            ...
            cursor.execute("""
                ...
                FROM chat_messages m
                LEFT JOIN chat_reads r ON m.id = r.message_id AND r.user_id = %s
                WHERE r.id IS NULL AND m.receiver_id IS NULL
            """, (user_id,))
            result["comune"] = cursor.fetchone()[0]
            ...
            if role == 'DM':
                cursor.execute("""
                    ...
                    WHERE r.id IS NULL AND m.is_secret = 0 AND (
                        (m.sender_id = %s AND u.role = 'GIOCATORE')
                        OR (m.receiver_id = %s AND u.role = 'GIOCATORE')
                    )
                ...
                cursor.execute("""
                    ...
                    FROM chat_messages m
                    LEFT JOIN chat_reads r ON m.id = r.message_id AND r.user_id = %s
                    WHERE r.id IS NULL AND m.is_secret = 0
                      AND ((m.sender_id = %s AND m.receiver_id IS NOT NULL)
                        OR (m.receiver_id = %s))
                """, (user_id, user_id, user_id))
            result["privati"] = cursor.fetchone()[0]

            # --- Chat Segrete ---
            if role == 'DM':
                # Il DM vede anche le segrete
                cursor.execute("""
                    SELECT COUNT(*)
                    FROM chat_messages m
                    LEFT JOIN chat_reads r ON m.id = r.message_id AND r.user_id = %s
                ...
                """, (user_id, user_id, user_id))
            result["segreti"] = cursor.fetchone()[0]
    ...
    def main_menu(self):
        while True:
            ...
            unread = self._count_unread_messages()
            notif = f" 💌 ({unread} nuovi messaggi)" if unread > 0 else ""
            print(f"\n=== MENU PRINCIPALE ({username} - {role}) {notif} ===")
            print("1.  🧝 Gestione PG")
            ...
            if choice == '1':
                ...
                while True:
                    ...
                    print("0. Torna indietro")
                    sub = input("Scegli un'opzione: ").strip()
                    if sub == '1':
                        self.view_chat()
                    elif sub == '2':
                        self.send_message()
