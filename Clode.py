...
class DeDToolGUI:
    ...
    def show_main_menu(self):
        ...
        sidebar.pack_propagate(False)  # Importante: mantiene la larghezza fissa
        
        # 🔹 MODIFICA: Usa grid per un layout più compatto
        row = 0
        ...
        ttk.Separator(sidebar, orient='horizontal').grid(row=row, column=0, sticky='ew', padx=10, pady=5)
        row += 1
        
        # Menu buttons - riduci il padding per risparmiare spazio
        menu_items = [
        ...
        # Pulsante chat
        chat_text = "💬 Chat"
        unread_total = self._count_unread_messages()
        if unread_total > 0:
    ...
    def show_banks_menu(self):
        ...
        # 🔹 AGGIUNTA: Pulsante Esporta Excel (solo per DM)
        if self.current_user['role'] == 'DM':
            ttk.Button(btn_frame, text="📊 Esporta Excel", command=self.export_to_excel).pack(side='left', padx=5)
    ...
    def show_chat(self):
        """Mostra il sistema chat con notifiche"""
        self.clear_content()
        ...
        unread_counts = self._count_unread_by_category()
        ...
        # 🔹 FUNZIONE per aggiornare i tab quando vengono selezionati
        def on_tab_changed(event):
            ...
            if "(" in tab_name:
                ...
                # 🔹 Aggiorna anche i contatori globali e il pulsante chat
                self.update_chat_button()
        
        notebook.bind("<<NotebookTabChanged>>", on_tab_changed)
        
        # 🔹 Carica automaticamente i messaggi quando si apre la chat
        def on_chat_opened():
            # Forza il caricamento del tab attivo
            on_tab_changed(None)
        
        # Esegui dopo un breve delay per assicurarsi che l'interfaccia sia pronta
        self.root.after(100, on_chat_opened)
    ...
    def load_chat_messages(self, text_widget, chat_type='comune'):
        """Carica i messaggi nella chat con evidenziazione nuovi messaggi"""
        try:
            cursor = self.db.cursor()
            ...
            # 🔹 Configura il tag per l'evidenziazione (PRIMA di inserire i messaggi)
            text_widget.tag_configure("new_message", background='#e8f5e8')  # Verde chiaro
            ...
            for msg in reversed(messages):
                ...
                # 🔹 SEPARA la parte timestamp/nome dalla parte messaggio
                timestamp_part = f"[{date_str}] {sender}: "
                message_part = f"{msg['message']}\n"
                
                # Inserisci prima il timestamp (senza evidenziazione)
                text_widget.insert(tk.END, timestamp_part)
                
                # Poi inserisci il messaggio (con o senza evidenziazione)
            ...
            # 🔹 AGGIORNA IL CONTATORE dopo aver letto i messaggi
            self.update_chat_button()
    ...
    def create_private_chat_interface(self, parent):
        ...
        def load_private_messages():
            ...
            receiver_id = users[user_combo.current()]['id']
            receiver_name = users[user_combo.current()]['username']
            ...
            try:
                ...
                # 🔹 Configura il tag per l'evidenziazione
                ...
                for msg in reversed(messages):
                    ...
                    if msg['sender_id'] == self.current_user['id']:
                        ...
                        timestamp_part = f"[{date_str}] {msg['sender_name']} → Tu: "
                        message_part = f"{msg['message']}\n"
                ...
                # Marca come letti e aggiorna contatore
                for msg in messages:
                    cursor.execute("""
                        INSERT IGNORE INTO chat_reads (user_id, message_id) VALUES (%s, %s)
                    """, (self.current_user['id'], msg['id']))
        ...
        # 🔹 CARICAMENTO AUTOMATICO quando si seleziona un utente
        ...
        def send_private_message():
            ...
            try:
                ...
                message_entry.delete(0, tk.END)
                load_private_messages()  # Ricarica automaticamente dopo l'invio
        ...
        # 🔹 RIMUOVI il pulsante "Carica Chat" - non serve più
        # ttk.Button(select_frame, text="📨 Carica Chat", command=load_private_messages).pack(side='left', padx=5)
        ...
        if self.current_user['role'] == 'DM':
            ...
            def load_all_secret_messages():
                try:
                    ...
                    # 🔹 Configura il tag per l'evidenziazione (PRIMA di inserire i messaggi)
                    messages_text.tag_configure("new_message", background='#e8f5e8')  # Verde chiaro
                    ...
                    for msg in reversed(messages):
                        ...
                        # 🔹 SEPARA la parte timestamp/nome dalla parte messaggio
                        timestamp_part = f"[{date_str}] {sender} → {receiver}: "
                        message_part = f"{msg['message']}\n"
            ...
            def load_secret_messages():
                ...
                receiver_name = players[player_combo.current()]['username']
            ...
            # 🔹 CARICAMENTO AUTOMATICO quando si seleziona un giocatore
            def on_player_selected(event):
                load_secret_messages()
            
            player_combo.bind('<<ComboboxSelected>>', on_player_selected)
            
            # 🔹 CORREZIONE: Definisci input_frame PRIMA di usarlo nelle funzioni
            input_frame = ttk.Frame(parent)
            input_frame.pack(fill='x', padx=10, pady=5)
            ...
            # 🔹 Carica automaticamente se c'è solo un giocatore
            if len(players) == 1:
    ...
    def _count_unread_by_category(self):
        ...
        try:
            ...
            """, (user_id,))
            result["comune"] = cursor.fetchone()['count']
                ...
                cursor.execute("""
                    ...
                    WHERE r.id IS NULL AND m.is_secret = 0
                      AND ((m.sender_id = %s AND m.receiver_id IS NOT NULL)
                        OR (m.receiver_id = %s))
                """, (user_id, user_id, user_id))
            result["privati"] = cursor.fetchone()['count']
            ...
            if role == 'DM':
                ...
                """, (user_id, user_id, user_id))
            result["segreti"] = cursor.fetchone()['count']
    ...
    def check_unread_messages(self):
        """Controlla se ci sono messaggi non letti e mostra notifica nel menu"""
        unread_total = self._count_unread_messages()
        
        # Aggiorna il testo del pulsante chat nel menu principale
    ...
    def update_chat_button(self):
        """Aggiorna il pulsante chat nel menu principale"""
        if hasattr(self, 'chat_button'):
            unread_total = self._count_unread_messages()
            if unread_total > 0:
                self.chat_button.config(text=f"💬 Chat ({unread_total})")
