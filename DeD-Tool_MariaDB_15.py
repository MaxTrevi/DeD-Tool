...
class DeDTool:
    ...
    def update_user(self):
        ...
        if new_username:
            ...
            self.cursor.execute("SELECT id FROM users WHERE username = %s", (new_username,))
        ...
        if new_password:
            update_data['password'] = new_password  # In app reali: hash!
        ...
        if update_data:
            try:
                ...
                values = list(update_data.values()) + [user_id_to_update]
    ...
    def remove_user(self):
        ...
        confirm = input(
            f"Sei sicuro di voler rimuovere l'utente '{selected_user['username']}' (ID: {selected_user['id']})?\n"
            "Questa operazione non elimina i PG associati, ma li disassocia dall'utente. (s/N): ").strip().lower()

        if confirm == 's':
            try:
                # 1. Disassocia i PG
                self.cursor.execute("UPDATE player_characters SET user_id = NULL WHERE user_id = %s", (user_id_to_remove,))
                print(f"🔄 PGs associati all'utente '{selected_user['username']}' disassociati.")

                # 2. Elimina l'utente
                self.cursor.execute("DELETE FROM users WHERE id = %s", (user_id_to_remove,))
                self.conn.commit()
    ...
    def add_objective_event(self):
        ...
        # 1. Obiettivi attivi
        self.cursor.execute("SELECT id, name, status, follower_id FROM follower_objectives WHERE status = %s", 
        ...
        try:
            self.cursor.execute("SELECT pg_id FROM followers WHERE id = %s", (selected_obj['follower_id'],))
            follower_data = self.cursor.fetchone()

            if follower_data:
                pg_id = follower_data['pg_id']
                self.cursor.execute("SELECT user_id FROM player_characters WHERE id = %s", (pg_id,))
                user_data = self.cursor.fetchone()

                if user_data:
                    user_id = user_data['user_id']
                    self.cursor.execute("SELECT mail FROM users WHERE id = %s", (user_id,))
    ...
    """
        ...
        try:
            response = client.chat.completions.create(
                model="mistral",  # nome accettato da LM Studio
